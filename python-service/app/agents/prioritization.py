from app.agents.audit_rules import evaluate_site_audit, findings_to_opportunities
...
def generate_site_opportunities(site_audit_result: dict, city: str | None = None, country: str | None = None) -> list[dict]:
    """
    Transforme uniquement des constats déterministes et prouvés en actions.
    Les contrôles réussis et non testés restent disponibles dans
    site_audit_result["detailed_findings"], mais ne deviennent jamais des
    opportunités.
    """
    findings = site_audit_result.get("detailed_findings")
    if not isinstance(findings, list):
        findings = evaluate_site_audit(site_audit_result, city, country)
    return findings_to_opportunities(findings, limit=5)
